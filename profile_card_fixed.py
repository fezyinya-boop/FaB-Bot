...
def draw_tracked_name(base_img, text_value, pos, font, tracking, fill=(236, 236, 240, 255),
    ...
    cx = x
    for i, ch in enumerate(chars):
        d.text((cx, y + 2), ch, font=font, fill=(0, 0, 0, 110))
        d.text((cx, y - 1), ch, font=font, fill=(255, 255, 255, 28))
...
# ----------------------------
def apply_carbon_fiber(base: Image.Image, p_x1: int, p_y1: int, p_x2: int, p_y2: int, radius: int, scale_fn) -> Image.Image:
    W, H = base.size
    ...
    # Dark cell base fill
    td.rectangle((0, 0, t_size - 1, t_size - 1), fill=(255, 255, 255, 6))
    # Cell border lines (the "weave" grid)
    td.line((0, 0, t_size, 0), fill=(0, 0, 0, 80), width=1)
    td.line((0, 0, 0, t_size), fill=(0, 0, 0, 80), width=1)
    # Two-tone diagonal highlight simulating fiber sheen
    td.line((0, 0, t_size // 2, t_size // 2), fill=(255, 255, 255, 55), width=1)
    td.line((t_size // 2, t_size // 2, t_size, t_size), fill=(255, 255, 255, 18), width=1)
...
def make_profile_card(
    ...
    # Composite panel onto card first, THEN apply carbon fiber on top
    ...
    # Subtle vertical gradient on panel
