...
def draw_tracked_name(
    ...
    if gradient_rgb is not None:
        ...
        # Soft rank-color glow composited beneath the gradient text.
        if rr + gg + bb > 0:
            glow_layer = Image.new("RGBA", base_img.size, (0, 0, 0, 0))
            gl = ImageDraw.Draw(glow_layer)
            cx2 = x
            for i, ch in enumerate(chars):
                gl.text((cx2, y), ch, font=font, fill=(rr, gg, bb, 180))
            ...
            # Clamp glow alpha so it stays a halo, not a colour flood
            glow_layer = glow_layer.point(lambda v: min(v, 110))
...
def make_profile_card(
    ...
    name_gradient_rgb = rc if _rank_clean in ("DIAMOND", "PLATINUM", "GOLD") else CARD_GOLD
    ...
    if badge:
        # Discord-style indicator: badge sits mostly outside avatar, top-left corner overlapping.
        badge_x = av_x + av_size - S(30)
        badge_y = av_y + av_size - S(30)
