...
def fit_cinzel_font(draw: ImageDraw.ImageDraw, text: str, max_w: int, start_size: int, min_size: int):
    size = start_size
    while size >= min_size:
        f = load_cinzel_font(size)
        if text_width(draw, text, f) <= max_w:
            return f
        size -= 1
    return load_cinzel_font(min_size)


def make_carbon_texture(width: int, height: int, scale: int) -> Image.Image:
    tex = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    d = ImageDraw.Draw(tex)
    cell = max(10, 16 * scale)
    half = cell // 2
    q = max(4, cell // 4)

    # dark-on-dark hex / weave hybrid
    for row, y in enumerate(range(0, height + cell, int(cell * 0.86))):
        ...
        for x in range(-cell, width + cell, cell):
            ...
            d.line(pts + [pts[0]], fill=(255, 255, 255, 9), width=max(1, scale))

    # subtle diagonal carbon-fiber strands
    step = max(8, 12 * scale)
    for i in range(-height, width, step):
        d.line((i, 0, i + height, height), fill=(255, 255, 255, 5), width=max(1, scale))
        d.line((i + step // 2, 0, i + step // 2 + height, height), fill=(0, 0, 0, 10), width=max(1, scale))

    tex = tex.filter(ImageFilter.GaussianBlur(radius=max(1, scale)))
    return tex


def draw_premium_name(base_img, text_value, pos, font, tracking, rank_color=(255, 214, 96)):
    x, y = pos
    chars = list(text_value)
    widths = []
    total_w = 0
    ...
    for i, ch in enumerate(chars):
        # drop shadow / anchor
        d.text((cx + 2, y + 3), ch, font=font, fill=(0, 0, 0, 210))
        # dark gold body
        d.text((cx, y + 1), ch, font=font, fill=(105, 72, 24, 255), stroke_width=2, stroke_fill=(0, 0, 0, 205))
        # bright metallic face
        d.text((cx, y), ch, font=font, fill=(245, 214, 132, 255), stroke_width=2, stroke_fill=(18, 10, 4, 210))
        # upper highlight
        d.text((cx, y - 2), ch, font=font, fill=(255, 247, 214, 72))
        cx += widths[i] + (tracking if i < len(chars) - 1 else 0)
    ...
    cx = x
    for i, ch in enumerate(chars):
        ad.text((cx, y), ch, font=font, fill=255, stroke_width=2, stroke_fill=255)
        cx += widths[i] + (tracking if i < len(chars) - 1 else 0)

    grad = Image.new("RGBA", base_img.size, (0, 0, 0, 0))
    gd = ImageDraw.Draw(grad)
    bbox = d.textbbox((x, y), text_value, font=font, stroke_width=2)
    top, bottom = bbox[1], bbox[3]
    shimmer_x = x + int(total_w * 0.58)
    for yy in range(top - 2, bottom + 2):
        ...
        if t < 0.35:
            ...
            c = (120, 76, 26, 80)
        gd.line((x - 4, yy, x + total_w + 6, yy), fill=c)

    # permanent metallic shimmer band
    for off in range(-18, 19):
        alpha_v = max(0, 52 - abs(off) * 2)
        gd.line((shimmer_x + off, top - 2, shimmer_x + off + 30, bottom + 2), fill=(255, 255, 235, alpha_v), width=1)

    grad.putalpha(alpha)
    out = Image.alpha_composite(overlay, grad)
    base_img.alpha_composite(out)
    return int(total_w)
...
# Square off the top by extending full width - removes rounded top corners
    ...
    # Carbon-fiber / hex micro texture with edge-catching light
    panel_texture = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    tex = make_carbon_texture(W, panel_y2 - panel_y, SCALE)
    tex_mask = Image.new("L", (W, H), 0)
    tmd = ImageDraw.Draw(tex_mask)
    tmd.rounded_rectangle((panel_x1, panel_y, panel_x2, panel_y2), radius=S(28), fill=255)
    panel_texture.paste(tex, (0, panel_y), tex)
    panel_texture.putalpha(ImageChops.multiply(panel_texture.split()[-1], tex_mask))
    card = Image.alpha_composite(card, panel_texture)

    # Razor-thin rim light along the top edge of the panel for layering
    rim = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    rd = ImageDraw.Draw(rim)
    rd.line((S(8), panel_y + S(2), W - S(8), panel_y + S(2)), fill=(255, 244, 214, 105), width=S(1))
    rd.line((S(10), panel_y + S(4), W - S(10), panel_y + S(4)), fill=(255, 210, 120, 38), width=S(1))
    rim = rim.filter(ImageFilter.GaussianBlur(radius=S(1)))
    ...
    # recessed inner shadow so the avatar feels seated inside a port
    recess = Image.new("RGBA", (av_size, av_size), (0, 0, 0, 0))
    rs = ImageDraw.Draw(recess)
    for i in range(S(18)):
        alpha = int(12 + (i * 4))
        rs.ellipse((i, i, av_size - i - 1, av_size - i - 1), outline=(0, 0, 0, alpha), width=1)
    recess = recess.filter(ImageFilter.GaussianBlur(radius=S(3)))
    av_circ = Image.alpha_composite(av_circ, recess)

    # paste avatar first
    card.paste(av_circ, (av_x, av_y), av_circ)

    # tactical metallic bezel assembly
    bezel = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    bzd = ImageDraw.Draw(bezel)

    # outer ambient glow
    for r in range(S(12), S(42), S(6)):
        a = max(0, 42 - r)
        bzd.ellipse((av_x - r, av_y - r, av_x + av_size + r, av_y + av_size + r), outline=(rc[0], rc[1], rc[2], a), width=S(3))

    # heavy matte gold primary bezel
    outer = (av_x - S(8), av_y - S(8), av_x + av_size + S(8), av_y + av_size + S(8))
    bzd.ellipse(outer, fill=(64, 45, 18, 215))
    bzd.ellipse((outer[0] + S(8), outer[1] + S(8), outer[2] - S(8), outer[3] - S(8)), fill=(0, 0, 0, 0))
    bzd.ellipse(outer, outline=(220, 170, 72, 255), width=S(6))
    bzd.arc(outer, start=205, end=330, fill=(255, 235, 168, 150), width=S(4))

    # secondary inner technical dial / reticle
    inner = (av_x + S(6), av_y + S(6), av_x + av_size - S(6), av_y + av_size - S(6))
    bzd.ellipse(inner, outline=(25, 22, 20, 240), width=S(8))
    dial = (av_x + S(14), av_y + S(14), av_x + av_size - S(14), av_y + av_size - S(14))
    bzd.ellipse(dial, outline=(212, 176, 98, 165), width=S(2))
    for ang in range(0, 360, 30):
        ...
        r2 = av_size / 2 + S(-16)
        a = math.radians(ang)
        x1 = cx2 + math.cos(a) * r1
        y1 = cy2 + math.sin(a) * r1
        x2 = cx2 + math.cos(a) * r2
        y2 = cy2 + math.sin(a) * r2
        bzd.line((x1, y1, x2, y2), fill=(180, 150, 82, 135), width=S(2))
    bzd.arc(dial, start=18, end=118, fill=(255, 240, 205, 90), width=S(2))
    bzd.arc(dial, start=198, end=300, fill=(255, 240, 205, 70), width=S(2))

    bezel = bezel.filter(ImageFilter.GaussianBlur(radius=S(1)))
    card = Image.alpha_composite(card, bezel)
    draw = ImageDraw.Draw(card)
    ...
    name_max_w = max(S(360), right_x - name_x - S(120))
    f_name = fit_cinzel_font(draw, name_text, max_w=name_max_w, start_size=S(58), min_size=S(30))
    ...
    f_pts = fit_font(draw, str(pts), "Orbitron-VariableFont_wght.ttf", max_w=S(320), start_size=S(82), min_size=S(48))
    f_rp = fit_font(draw, "RP", "Orbitron-VariableFont_wght.ttf", max_w=S(90), start_size=S(44), min_size=S(24))
    pts_text = str(pts)
    pts_w = text_width(draw, pts_text, f_pts)
    rp_x = name_x + pts_w + S(18)

    rp_overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    rpd = ImageDraw.Draw(rp_overlay)
    rpd.text((name_x, rating_value_y + S(4)), pts_text, font=f_pts, fill=(0, 0, 0, 120))
    rpd.text((rp_x, rating_value_y + S(20)), "RP", font=f_rp, fill=(0, 0, 0, 105))

    pts_alpha = Image.new("L", (W, H), 0)
    pad = ImageDraw.Draw(pts_alpha)
    pad.text((name_x, rating_value_y), pts_text, font=f_pts, fill=255)
    pad.text((rp_x, rating_value_y + S(16)), "RP", font=f_rp, fill=215)

    gold_grad = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    ggd = ImageDraw.Draw(gold_grad)
    text_h = max(S(86), getattr(f_pts, "size", S(60)) + S(14))
    for i in range(text_h):
        ...
        if t < 0.35:
            ...
            col = (164, 108, 42, 255)
        ggd.line((name_x - S(4), rating_value_y + i, name_x + S(430), rating_value_y + i), fill=col)
    gold_grad.putalpha(pts_alpha)
