...
def draw_tracked_name(base_img, text_value, pos, font, tracking, fill=(236, 236, 240, 255),
    ...
    cx = x
    for i, ch in enumerate(chars):
        d.text((cx, y + 3), ch, font=font, fill=(0, 0, 0, 135))
        d.text((cx, y + 1), ch, font=font, fill=(165, 118, 46, 235), stroke_width=max(1, stroke_width), stroke_fill=(0, 0, 0, 190))
        d.text((cx, y), ch, font=font, fill=(255, 222, 132, 255), stroke_width=max(1, stroke_width), stroke_fill=(6, 6, 10, 220))
        d.text((cx, y - 2), ch, font=font, fill=(255, 248, 220, 120))
        cx += widths[i] + (tracking if i < len(chars) - 1 else 0)

    alpha = Image.new("L", base_img.size, 0)
    ad = ImageDraw.Draw(alpha)
    cx = x
    for i, ch in enumerate(chars):
        ad.text((cx, y), ch, font=font, fill=255, stroke_width=max(1, stroke_width), stroke_fill=255)
        cx += widths[i] + (tracking if i < len(chars) - 1 else 0)

    shimmer = Image.new("RGBA", base_img.size, (0, 0, 0, 0))
    sd = ImageDraw.Draw(shimmer)
    shimmer_w = max(36, int(total_w * 0.22))
    for dx in range(shimmer_w):
        t = dx / max(1, shimmer_w - 1)
        a = int(128 * (1 - abs(t - 0.5) * 2))
        sx = x + int(total_w * 0.18) + dx
        sd.line((sx, y - 8, sx, y + font.size + 14), fill=(255, 245, 220, a), width=2)
    shimmer.putalpha(ImageChops.multiply(shimmer.getchannel("A"), alpha))
    base_img.alpha_composite(shimmer)
    ...
    try:
        ...
        if isinstance(avatar_img, str):
            if os.path.exists(avatar_img):
                return Image.open(avatar_img).convert("RGBA")
            if avatar_img.startswith(("http://", "https://")):
                with urllib.request.urlopen(avatar_img, timeout=6) as resp:
...
def add_carbon_fiber_texture(base: Image.Image, panel_y: int, panel_y2: int, scale_fn) -> Image.Image:
    W, H = base.size
    tex = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    td = ImageDraw.Draw(tex)
    cell = max(scale_fn(10), 12)
    row_h = max(1, int(cell * 0.86))

    # dark-on-dark hex weave with slightly stronger edge catch so it survives downscaling
    for y in range(panel_y + scale_fn(10), panel_y2 - scale_fn(12), row_h):
        odd = ((y // row_h) % 2) == 1
        for x in range((-cell // 2) if odd else 0, W + cell, cell):
            x0 = x
            x1 = x + cell // 2
            x2 = x + cell
            cy = y + row_h // 2
            td.polygon([(x1, y), (x2, cy), (x1, y + row_h), (x0, cy)], outline=(255, 255, 255, 20))
            td.line((x0, cy, x1, y), fill=(0, 0, 0, 22), width=max(1, scale_fn(1)))
            td.line((x1, y + row_h, x2, cy), fill=(0, 0, 0, 18), width=max(1, scale_fn(1)))

    # subtle diagonal carbon sweep
    step = max(scale_fn(24), 20)
    for x in range(-H, W, step):
        td.line((x, panel_y, x + H // 3, panel_y2), fill=(255, 255, 255, 8), width=max(1, scale_fn(1)))
    for x in range(0, W + H, step):
        td.line((x, panel_y, x - H // 3, panel_y2), fill=(0, 0, 0, 10), width=max(1, scale_fn(1)))

    tex = tex.filter(ImageFilter.GaussianBlur(radius=max(1, scale_fn(1))))
    return Image.alpha_composite(base, tex)


def draw_tactical_avatar_frame(card: Image.Image, av_x: int, av_y: int, av_size: int, scale_fn) -> None:
    shadow = Image.new("RGBA", card.size, (0, 0, 0, 0))
    sd = ImageDraw.Draw(shadow)
    for i in range(scale_fn(14)):
        a = int(14 * (1 - i / max(1, scale_fn(14))))
        sd.ellipse((av_x + i, av_y + i, av_x + av_size - i, av_y + av_size - i), outline=(0, 0, 0, a), width=2)
    shadow = shadow.filter(ImageFilter.GaussianBlur(radius=scale_fn(2)))
    card.alpha_composite(shadow)

    d = ImageDraw.Draw(card)
    d.ellipse((av_x - scale_fn(10), av_y - scale_fn(10), av_x + av_size + scale_fn(10), av_y + av_size + scale_fn(10)), fill=(25, 22, 18, 255), outline=(194, 145, 62, 255), width=scale_fn(5))
    d.ellipse((av_x - scale_fn(6), av_y - scale_fn(6), av_x + av_size + scale_fn(6), av_y + av_size + scale_fn(6)), outline=(255, 235, 170, 55), width=scale_fn(2))
    d.arc((av_x - scale_fn(10), av_y - scale_fn(10), av_x + av_size + scale_fn(10), av_y + av_size + scale_fn(10)), start=205, end=312, fill=(255, 245, 205, 120), width=scale_fn(3))
    d.ellipse((av_x - scale_fn(14), av_y - scale_fn(14), av_x + av_size + scale_fn(14), av_y + av_size + scale_fn(14)), outline=(94, 70, 30, 210), width=scale_fn(2))

    import math
    cx = av_x + av_size // 2
    cy = av_y + av_size // 2
    r1 = av_size // 2 + scale_fn(2)
    r2 = av_size // 2 + scale_fn(12)
    for ang in range(0, 360, 30):
        ...
        d.line((x1, y1, x2, y2), fill=(145, 116, 58, 120), width=scale_fn(2))
...
# Square off the top by extending full width - removes rounded top corners
    ...
    card = add_carbon_fiber_texture(card, panel_y, panel_y2, S)
    draw = ImageDraw.Draw(card)
    draw.line((0, panel_y + S(2), W, panel_y + S(2)), fill=(255, 235, 190, 48), width=S(1))
    ...
    # edge-catching pass so the weave is visible after final resize
    weave_edge = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    wed = ImageDraw.Draw(weave_edge)
    for y in range(panel_y + S(20), panel_y2, S(34)):
        alpha = 10 if (y // S(34)) % 2 == 0 else 6
        wed.line((S(18), y, W - S(18), y), fill=(255, 255, 255, alpha), width=S(1))
    card = Image.alpha_composite(card, weave_edge)
    draw = ImageDraw.Draw(card)
    ...
    if avatar_has_real_image:
        ...
        avatar_img = Image.new("RGBA", (av_size, av_size), (40, 28, 18, 255))
    ...
    recessed = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    rd = ImageDraw.Draw(recessed)
    for i in range(S(10)):
        a = int(26 * (1 - i / max(1, S(10))))
        rd.ellipse((av_x + i, av_y + i, av_x + av_size - i, av_y + av_size - i), outline=(0, 0, 0, a), width=max(1, S(2)))
    recessed = recessed.filter(ImageFilter.GaussianBlur(radius=S(2)))
    card = Image.alpha_composite(card, recessed)

    # paste clean avatar
    card.paste(av_circ, (av_x, av_y), av_circ)
    draw_tactical_avatar_frame(card, av_x, av_y, av_size, S)
    ...
    name_max_w = max(S(280), right_x - name_x - S(92))
    name_size = S(58)
    while name_size >= S(34):
        f_name = load_cinzel_font(name_size)
        tracking_test = max(S(3), int(name_size * 0.16))
        est_w = sum(int(f_name.getlength(ch)) for ch in name_text) + max(0, len(name_text)-1) * tracking_test
        if est_w <= name_max_w:
            break
        name_size -= S(2)
    f_name = load_cinzel_font(name_size)
    name_tracking = max(S(3), int(name_size * 0.16))

    rp_num_font = fit_font(draw, f"{pts}", "Orbitron-VariableFont_wght.ttf", max_w=S(270), start_size=S(70), min_size=S(42))
    rp_unit_font = fit_font(draw, "RP", "Orbitron-VariableFont_wght.ttf", max_w=S(90), start_size=max(S(26), int(rp_num_font.size * 0.48)), min_size=S(22))
    ...
    rgd.rounded_rectangle(
        (name_x - S(18), rating_value_y - S(18), name_x + S(380), rating_value_y + S(94)),
        radius=S(24),
    ...
    # Number and smaller RP rendered separately
    num_text = f"{pts}"
    draw.text((name_x, rating_value_y + S(4)), num_text, font=rp_num_font, fill=(0, 0, 0, 120))
    draw.text((name_x, rating_value_y), num_text, font=rp_num_font, fill=(255, 235, 170, 255), stroke_width=S(2), stroke_fill=(0, 0, 0, 120))
    num_w = int(draw.textlength(num_text, font=rp_num_font))
    rp_x = name_x + num_w + S(20)
    rp_y = rating_value_y + max(0, rp_num_font.size - rp_unit_font.size - S(2))
    draw.text((rp_x, rp_y + S(3)), "RP", font=rp_unit_font, fill=(0, 0, 0, 110))
    draw.text((rp_x, rp_y), "RP", font=rp_unit_font, fill=(236, 192, 102, 255), stroke_width=S(1), stroke_fill=(0, 0, 0, 90))
