...
def draw_tracked_name(
    ...
    if gradient_rgb is not None:
        ...
        # Soft specular sheen — kept subtle to avoid banding artifacts.
        shine_y = top_y + max(1, int(text_h * 0.17))
        shine_w = max(1, font.size // 16)
        gd.line((left_x, shine_y, right_x, shine_y), fill=(255, 255, 255, 28), width=shine_w)
        gd.line((left_x, shine_y + shine_w, right_x, shine_y + shine_w), fill=(255, 255, 255, 10), width=max(1, shine_w // 2))
        flare_y = top_y + max(1, int(text_h * 0.34))
        gd.line((left_x, flare_y, right_x, flare_y), fill=(255, 255, 255, 8), width=max(1, font.size // 26))
...
def make_profile_card(
    ...
    name_tracking = S(16)
    name_max_w = right_x - name_x - S(80)
    ...
    if badge:
        # Place badge so its top-left barely overlaps the bottom-right of the avatar ring.
        badge_x = av_x + av_size - S(14)
        badge_y = av_y + av_size - S(14)
