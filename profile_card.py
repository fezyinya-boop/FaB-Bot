...
def add_panel_texture(base_img, panel_box, scale_fn):
    ...
    # faint hex / console style pattern
    step_x = S(22)
    step_y = S(12)
    ...
    for y in range(0, h + step_y, step_y):
        ...
        for x in range(-step_x, w + step_x, step_x):
            ...
            pts = [
                (cx, y),
                (cx + step_x // 3, y - step_y // 2),
                (cx + 2 * step_x // 3, y - step_y // 2),
                (cx + step_x, y),
                (cx + 2 * step_x // 3, y + step_y // 2),
                (cx + step_x // 3, y + step_y // 2),
            ]
            td.line(pts + [pts[0]], fill=line_col, width=1)

    # faint top-to-bottom sheen
    sheen = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    sd = ImageDraw.Draw(sheen)
    for yy in range(h):
        a = int(10 * (1 - yy / max(1, h)))
        sd.line((0, yy, w, yy), fill=(255, 255, 255, a), width=1)

    tex = Image.alpha_composite(tex, sheen)
    base_img.alpha_composite(tex, (x1, y1))
...
# Square off the top by extending full width - removes rounded top corners
    ...
    add_panel_texture(card, (panel_x1, panel_y, panel_x2, panel_y2), S)
    draw = ImageDraw.Draw(card)
    ...
    # subtle avatar scanlines for archive-tech styling
    if avatar_has_real_image:
        avatar_fx = Image.new("RGBA", (av_size, av_size), (0, 0, 0, 0))
        afd = ImageDraw.Draw(avatar_fx)
        for sy in range(0, av_size, S(6)):
            afd.line((0, sy, av_size, sy), fill=(255, 255, 255, 10), width=1)
    ...
    f_big_num = fit_font(draw, rp_num, "Orbitron-VariableFont_wght.ttf", max_w=S(280), start_size=S(84), min_size=S(48))
    f_big_rp = fit_font(draw, rp_suffix, "Orbitron-VariableFont_wght.ttf", max_w=S(120), start_size=S(56), min_size=S(30))

    num_bbox = draw.textbbox((0, 0), rp_num, font=f_big_num)
    rp_bbox = draw.textbbox((0, 0), rp_suffix, font=f_big_rp)
    num_w = num_bbox[2] - num_bbox[0]
    rp_w = rp_bbox[2] - rp_bbox[0]
    gap = S(14)
    total_rp_w = num_w + gap + rp_w

    rp_overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    rpd = ImageDraw.Draw(rp_overlay)

    # shadow
    rpd.text((name_x + S(3), rating_value_y + S(5)), rp_num, font=f_big_num, fill=(0, 0, 0, 120))
    rpd.text((name_x + num_w + gap + S(3), rating_value_y + S(18)), rp_suffix, font=f_big_rp, fill=(0, 0, 0, 110))

    # highlight
    rpd.text((name_x, rating_value_y), rp_num, font=f_big_num, fill=(255, 245, 190, 255))
    rpd.text((name_x + num_w + gap, rating_value_y + S(13)), rp_suffix, font=f_big_rp, fill=(255, 235, 170, 245))

    rp_alpha = Image.new("L", (W, H), 0)
    alpha_draw = ImageDraw.Draw(rp_alpha)
    alpha_draw.text((name_x, rating_value_y), rp_num, font=f_big_num, fill=255)
    alpha_draw.text((name_x + num_w + gap, rating_value_y + S(13)), rp_suffix, font=f_big_rp, fill=255)

    gold_grad = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    for i in range((num_bbox[3] - num_bbox[1]) + S(18)):
        t = i / max(1, (num_bbox[3] - num_bbox[1]) + S(18))
        r = int(255 * (1 - t) + 185 * t)
        g = int(240 * (1 - t) + 165 * t)
        b = int(185 * (1 - t) + 70 * t)
        ImageDraw.Draw(gold_grad).line((name_x, rating_value_y + i, name_x + total_rp_w + S(6), rating_value_y + i), fill=(r, g, b, 255))
