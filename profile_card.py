...
def draw_tracked_name(base_img, text_value, pos, font, tracking,
    ...
    for i, ch in enumerate(chars):
        ...
        d.text((cx + 2, y + 3), ch, font=font, fill=(0, 0, 0, 140))
        # Mid shadow
        d.text((cx + 1, y + 1), ch, font=font, fill=(0, 0, 0, 70))
        # Main silver body - pure bright silver
        d.text((cx, y), ch, font=font,
               fill=(235, 238, 245, 255),
               stroke_width=stroke_width,
               stroke_fill=(60, 65, 80, 140))
        # Strong specular highlight - top edge
        d.text((cx, y - 1), ch, font=font, fill=(255, 255, 255, 180))
        # Secondary shimmer
        d.text((cx - 1, y), ch, font=font, fill=(210, 220, 240, 60))
...
def make_profile_card(
    ...
    rgd.text((name_x + S(3), rating_value_y + S(4)), num_text, font=f_big,
             fill=(rc[0], rc[1], rc[2], 50), stroke_width=S(2), stroke_fill=(0, 0, 0, 0))
    rgd.text((rp_x + S(3), rp_y + S(4)), "RP", font=f_rp,
             fill=(rc[0], rc[1], rc[2], 35), stroke_width=S(1), stroke_fill=(0, 0, 0, 0))
