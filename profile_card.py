...
def draw_tracked_name(base_img, text_value, pos, font, tracking, fill=(250, 250, 255, 255),
                      stroke_fill=(0, 0, 0, 145), stroke_width=1,
                      glow_fill=(255, 255, 255, 0), underline_fill=(255, 200, 90, 0),
                      underline_offset=58, underline_width=0):
    x, y = pos
    ...
    cx = x
    for i, ch in enumerate(chars):
        # sharp drop shadow for physical presence
        d.text((cx + 2, y + 2), ch, font=font, fill=(0, 0, 0, 115))
        # main silver text
        d.text(
            (cx, y),
            ch,
        ...
        # subtle top highlight for metallic feel
        d.text((cx, y - 1), ch, font=font, fill=(255, 255, 255, 42))
...
# Square off the top by extending full width - removes rounded top corners
    ...
    # Carbon fiber hexagonal background at 5% opacity on dark panel area
    import math
    hex_layer = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    hd = ImageDraw.Draw(hex_layer)
    hex_r = S(18)  # hex radius
    hex_h = int(hex_r * math.sqrt(3))
    hex_line_col = (255, 255, 255, 13)  # ~5% opacity
    col_w = hex_r * 3
    row_h = hex_h
    for row in range(-1, H // row_h + 2):
        for col in range(-1, W // col_w + 2):
            cx_h = col * col_w + (hex_r if row % 2 == 1 else 0)
            cy_h = row * row_h + panel_y
            pts_hex = []
            for angle_deg in range(0, 360, 60):
                angle_rad = math.radians(angle_deg + 30)
                px = cx_h + hex_r * math.cos(angle_rad)
                py = cy_h + hex_r * math.sin(angle_rad)
                pts_hex.append((px, py))
            hd.polygon(pts_hex, outline=hex_line_col)
    card = Image.alpha_composite(card, hex_layer)
    draw = ImageDraw.Draw(card)
    ...
    # Stylized avatar border - outer glow halo
    draw.ellipse(
        (av_x - S(18), av_y - S(18), av_x + av_size + S(18), av_y + av_size + S(18)),
        outline=(rc[0], rc[1], rc[2], 28),
        width=S(10),
    )
    # Mid diffuse ring
    draw.ellipse(
        (av_x - S(11), av_y - S(11), av_x + av_size + S(11), av_y + av_size + S(11)),
        outline=(rc[0], rc[1], rc[2], 80),
        width=S(5),
    )
    # Main crisp gold ring
    draw.ellipse(
        (av_x - S(6), av_y - S(6), av_x + av_size + S(6), av_y + av_size + S(6)),
        outline=(rc[0], rc[1], rc[2], 255),
        width=S(4),
    )
    # Inner thin accent ring - dark separator
    draw.ellipse(
        (av_x - S(2), av_y - S(2), av_x + av_size + S(2), av_y + av_size + S(2)),
        outline=(0, 0, 0, 180),
        width=S(2),
    )
    # Specular highlight arc (top-left bright spot)
    draw.arc(
        (av_x - S(6), av_y - S(6), av_x + av_size + S(6), av_y + av_size + S(6)),
        start=200, end=310,
    ...
    name_max_w = max(S(220), right_x - name_x - S(100))
    f_name = fit_font(draw, name_text, "Cinzel-Bold.ttf", max_w=name_max_w, start_size=S(44), min_size=S(24))

    # Metallic sheen name rendering
    name_layer = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    nd = ImageDraw.Draw(name_layer)

    # Subtle glow pass
    glow_layer = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    gd_name = ImageDraw.Draw(glow_layer)
    gd_name.text((name_x, name_y - S(4)), name_text, font=f_name, fill=(200, 215, 255, 35))
    glow_layer = glow_layer.filter(ImageFilter.GaussianBlur(radius=S(6)))
    card = Image.alpha_composite(card, glow_layer)
    draw = ImageDraw.Draw(card)

    # Drop shadow
    nd.text((name_x + S(2), name_y - S(2)), name_text, font=f_name, fill=(0, 0, 0, 130), stroke_width=S(1), stroke_fill=(0,0,0,0))
    # Base silver
    nd.text((name_x, name_y - S(4)), name_text, font=f_name, fill=(210, 215, 230, 255), stroke_width=S(1), stroke_fill=(8, 8, 12, 120))

    # Metallic gradient sheen overlay (light top, darker bottom)
    name_bbox_m = nd.textbbox((name_x, name_y - S(4)), name_text, font=f_name)
    name_h_m = name_bbox_m[3] - name_bbox_m[1]
    for i in range(name_h_m):
        t = i / max(1, name_h_m - 1)
        # Bright silver at top, mid-tone at bottom, bright again at 70%
        if t < 0.3:
            lum = int(255 * (1 - t / 0.3) * 0.45 + 180)
        elif t < 0.7:
            lum = int(180 - (t - 0.3) / 0.4 * 40)
        else:
            lum = int(140 + (t - 0.7) / 0.3 * 60)
        alpha = int(90 * (1 - abs(t - 0.5) * 1.5))
        nd.line((name_x, name_bbox_m[1] + i, name_x + name_max_w, name_bbox_m[1] + i),
                fill=(lum, lum, lum+10, max(0, alpha)))

    # Top highlight edge
    nd.text((name_x, name_y - S(5)), name_text, font=f_name, fill=(255, 255, 255, 55))

    card = Image.alpha_composite(card, name_layer)
    draw = ImageDraw.Draw(card)

    tracked_name_w = text_width(draw, name_text, f_name)
    name_bbox = draw.textbbox((name_x, name_y - S(6)), name_text, font=f_name, stroke_width=S(1))
    badge = get_rank_badge(current_rank_raw, size=S(47))
    if badge:
        badge_y = name_y + (name_bbox[3] - name_bbox[1]) // 2 - badge.size[1] // 2 + 16
        badge_x = min(name_x + tracked_name_w + S(10), right_x - badge.size[0] - S(24))
    ...
    # Split pts number and "RP" label - number pops, RP is small
    pts_str = str(pts)
    f_pts = fit_font(draw, pts_str, "Orbitron-VariableFont_wght.ttf", max_w=S(340), start_size=S(76), min_size=S(48))
    f_rp_small = load_font("Orbitron-VariableFont_wght.ttf", S(28))

    pts_w = text_width(draw, pts_str, f_pts)
    rp_x = name_x + pts_w + S(10)
    # Vertical align "RP" to bottom of number
    pts_bbox = draw.textbbox((name_x, rating_value_y), pts_str, font=f_pts)
    rp_y = pts_bbox[3] - draw.textbbox((0,0), "RP", font=f_rp_small)[3] - S(4)
    ...
    rgd.text((name_x + S(3), rating_value_y + S(4)), pts_str, font=f_pts, fill=(rc[0], rc[1], rc[2], 100))
    rp_glow = rp_glow.filter(ImageFilter.GaussianBlur(radius=S(14)))
    card = Image.alpha_composite(card, rp_glow)
    draw = ImageDraw.Draw(card)

    # Draw pts number with gold gradient
    rp_overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    rpd = ImageDraw.Draw(rp_overlay)
    rpd.text((name_x, rating_value_y + S(4)), pts_str, font=f_pts, fill=(0, 0, 0, 120))
    rpd.text((name_x, rating_value_y), pts_str, font=f_pts, fill=(255, 245, 190, 255))

    rp_alpha = Image.new("L", (W, H), 0)
    rad = ImageDraw.Draw(rp_alpha)
    rad.text((name_x, rating_value_y), pts_str, font=f_pts, fill=255)
    gold_grad = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    ggd = ImageDraw.Draw(gold_grad)
    text_h = max(S(80), f_pts.size + S(10))
    ...
    # Draw small "RP" suffix - muted gold, bottom-aligned
    draw.text((rp_x, rp_y), "RP", font=f_rp_small, fill=(rc[0], rc[1], rc[2], 180), stroke_width=S(1), stroke_fill=(0, 0, 0, 120))
