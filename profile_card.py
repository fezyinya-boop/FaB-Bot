...
def draw_tracked_name(
    ...
    if gradient_rgb is not None:
        ...
        # Stronger but still controlled specular treatment.
        shine_y = top_y + max(1, int(text_h * 0.17))
        shine_w = max(1, font.size // 16)
        gd.line((left_x, shine_y, right_x, shine_y), fill=(255, 255, 255, 96), width=shine_w)
        gd.line((left_x, shine_y + shine_w, right_x, shine_y + shine_w), fill=(255, 255, 255, 34), width=max(1, shine_w // 2))
        flare_y = top_y + max(1, int(text_h * 0.34))
        gd.line((left_x, flare_y, right_x, flare_y), fill=(255, 255, 255, 26), width=max(1, font.size // 26))
...
def make_profile_card(
    ...
    name_tracking = S(16)
    name_max_w = right_x - name_x - S(16)
    ...
    badge = get_rank_badge(current_rank_raw, size=S(62))
    if badge:
        # Place badge on the lower-right of the avatar like a status indicator with slight overlap.
        badge_x = av_x + av_size - int(badge.size[0] * 0.42)
        badge_y = av_y + av_size - int(badge.size[1] * 0.42)

        badge_shadow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        bsd = ImageDraw.Draw(badge_shadow)
        bsd.ellipse(
            (badge_x + S(5), badge_y + S(7), badge_x + badge.size[0] - S(3), badge_y + badge.size[1] - S(1)),
            fill=(0, 0, 0, 82),
        )
        badge_shadow = badge_shadow.filter(ImageFilter.GaussianBlur(radius=S(4)))
        card = Image.alpha_composite(card, badge_shadow)
        draw = ImageDraw.Draw(card)

        # Small dark seat and highlight ring so the badge feels anchored to the avatar rim.
        seat = (badge_x - S(3), badge_y - S(3), badge_x + badge.size[0] + S(3), badge_y + badge.size[1] + S(3))
        draw.ellipse(seat, fill=(10, 10, 14, 230))
        draw.ellipse(seat, outline=(255, 255, 255, 34), width=S(1))
