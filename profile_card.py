...
def make_profile_card(
    ...
    f_big = fit_font(draw, f"{pts} RP", "Orbitron-VariableFont_wght.ttf", max_w=S(330), start_size=S(62), min_size=S(40))
    ...
    f_move = fit_font(draw, move_text, "Inter-VariableFont_opsz,wght.ttf", text_max_w, S(18), S(12))
    move_lines = wrap_text(draw, move_text, f_move, text_max_w, max_lines=3)
    line_h = max(S(18), (getattr(f_move, "size", S(14)) + S(4)))
    text_y = sig_y + S(40)
    ...
    content_top = panel_y + S(52)
    left_x = col_left
    right_x = col_left + int((col_right - col_left) * 0.68)
    row1_y = content_top + S(78)
    row2_y = row1_y + S(170)
    ...
    stat_block(left_x, row2_y, "", f"{wr}%", value_font=f_value, value_fill=WHITE)
    streak_fill = (rc[0], rc[1], rc[2], 255) if streak >= 3 else WHITE
    streak_text = f"+{streak}" if streak > 0 else str(streak)
    stat_block(right_x, row2_y, "", streak_text, value_font=f_value, value_fill=streak_fill)

    # Progress/footer strip
    pct_clamped = max(0.0, min(1.0, float(pct)))
    footer_label = "Archive Arena • Ranked Profile"
    if next_rank_raw:
        footer_label = f"{int(pct_clamped * 100)}% to {clean_rank_name(next_rank_raw)}"
    else:
        footer_label = "MAX RANK REACHED"
    ...
    bar_y = panel_y2 - S(50)
    label_y = bar_y - S(28)
    draw.text((col_left, label_y), footer_label, font=f_small, fill=GOLD if next_rank_raw else (rc[0], rc[1], rc[2], 255), stroke_width=S(1), stroke_fill=(0, 0, 0, 120))
