...
def draw_tracked_name(
    ...
    if gradient_rgb is not None:
        ...
        # Softer specular treatment: broad reflective bands instead of thin hard lines.
        shine = Image.new("RGBA", base_img.size, (0, 0, 0, 0))
        ...
        for i in range(text_h):
            ...
            if 0.08 <= t <= 0.34:
                u = (t - 0.08) / 0.26
                alpha = int(74 * (1 - abs((u * 2) - 1)) ** 1.5)
            elif 0.34 < t <= 0.48:
                u = (t - 0.34) / 0.14
                alpha = int(18 * (1 - u))
            if alpha > 0:
                sd.line((left_x, top_y + i, right_x, top_y + i), fill=(255, 255, 255, alpha))

        # Small top-edge sheen for vibrancy, kept soft so it reads as polish, not glitch.
        for i in range(max(1, text_h // 10)):
            alpha = int(26 * (1 - (i / max(1, text_h // 10))))
            sd.line((left_x, top_y + i, right_x, top_y + i), fill=(255, 255, 255, alpha))

        shine.putalpha(mask)
        shine = shine.filter(ImageFilter.GaussianBlur(radius=max(1, font.size // 30)))
...
def make_profile_card(
    ...
    name_tracking = S(16)
    name_max_w = right_x - name_x - S(54)
    ...
    if badge:
        ...
        badge_x = av_x + av_size - int(badge.size[0] * 0.54)
        badge_y = av_y + av_size - int(badge.size[1] * 0.54)

        # Very soft drop shadow only; no dark backing plate.
        badge_shadow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        bsd = ImageDraw.Draw(badge_shadow)
        bsd.ellipse(
            (badge_x + S(6), badge_y + S(7), badge_x + badge.size[0] - S(8), badge_y + badge.size[1] - S(6)),
            fill=(0, 0, 0, 54),
        )
        badge_shadow = badge_shadow.filter(ImageFilter.GaussianBlur(radius=S(5)))
