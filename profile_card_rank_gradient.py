...
def draw_tracked_name(
    ...
    if gradient_rgb is not None:
        ...
        top_col = (
            min(255, int(rr + (255 - rr) * 0.62)),
            min(255, int(gg + (255 - gg) * 0.62)),
            min(255, int(bb + (255 - bb) * 0.62)),
        )
        mid_col = (
            min(255, int(rr + (255 - rr) * 0.20)),
            min(255, int(gg + (255 - gg) * 0.20)),
            min(255, int(bb + (255 - bb) * 0.20)),
        )
        bot_col = (
            max(0, int(rr * 0.78)),
            max(0, int(gg * 0.78)),
        ...
        # Narrow specular band for a "shiny" read without reintroducing emboss.
        shine_y = top_y + max(1, int(text_h * 0.18))
        gd.line((left_x, shine_y, right_x, shine_y), fill=(255, 255, 255, 58), width=max(1, font.size // 18))
